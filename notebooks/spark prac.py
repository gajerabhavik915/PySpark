...
user.select('*', explode('Phone Number')). \
...
user.select('*', explode_outer('Phone Number')). \
   withColumnRenamed('key','phone type'). \
   withColumnRenamed('value','Phone NUM'). \
   drop('Phone Number'). \
   show()
...
user = spark.createDataFrame([Row(**user2) for user2 in users])
...
user.select('id', 'Phone Number.home_number', 'Phone Number.office_number').show()
...
user.select('id', col('Phone Number')['home_number'], col('Phone Number')['office_number']).show()
